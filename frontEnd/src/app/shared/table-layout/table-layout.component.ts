...
export class TableLayoutComponent implements OnInit, OnChanges, DoCheck, OnDestroy {
  ...
  @Output() productInStock = new EventEmitter<any>();
  ...
  onOpenEdit(record) {
    ...
    if (this.recordType === 'product') {
      ...
      modalRef.componentInstance.record = record;
      this.firstModalSubscription = modalRef.componentInstance.onProductInStock
        .subscribe(
          result => {
            console.log('here is the accepted result from table layout: ', result);
            this.productInStock.emit(result);
          }
        );
      this.secondModalSubscription = modalRef.componentInstance.onProductRenewed
        .subscribe(
          result => {
            console.log('here is the accepted result from table layout: ', result);
            this.productRenewed.emit(result);
          }
        );
    }
